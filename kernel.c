...
int do_exec(int b)
{
	...
	for (i = 1; i < 13; i++)	// i from 1 to 12
	{
		...
		switch (i)	// Set word that will be passed into put_word()
		{
			...
			case 10 : word = 0;			// di
				break;
			case 11 : word = running->uss;	// uES
				break;
			case 12 : word = running->uss;	// uDS
				running->usp = offset;	// Set PROC->uSS
				break;
		}
		put_word(word, running->uss, offset); // write the WORD to whatever SEGMENT, relative to the OFFSET
	}
...
int do_ps()
{
	int i;
	PROC *tmp;
	// freelist, readyqueue, sleeplist and PROCs
	printf("Free List:\t");
	printQueue(&freeList);
	printf("Ready Queue:\t");
	...
	while (i < NPROC)
	{
		...
		printf("ppid = ");
		if ((tmp->ppid > 0) && (tmp->ppid != tmp->pid))
			printf("%d, ", tmp->ppid);
		else
			printf(" , ");
		printf("event = ");
